...
import { Menu, X, ArrowRight, ChevronRight, Bell, ShoppingBag, Tag, Info, Trash2, CheckCheck, Sparkles, ShoppingCart } from 'lucide-react';
...
export const Header: React.FC<HeaderProps> = ({ 
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${isScrolled ? 'glass-morphism py-3 shadow-md' : 'bg-transparent py-5'}`}>
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="hidden md:flex items-center space-x-8">
            <button 
              ...
              className="text-lg font-black text-slate-700 hover:text-[#F7941D] transition-all duration-300 relative group"
            >
              About
              <span className="absolute -bottom-1 left-0 w-0 h-1 bg-[#F7941D] transition-all duration-300 group-hover:w-full"></span>
            </button>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="relative" ref={notificationRef}>
                <button 
                  ...
                  className={`p-3 rounded-full transition-all relative ${showNotifications ? 'bg-[#0F3D2E] text-white' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}
                >
              ...
              {/* Shopping Cart Icon - NOW CLICKABLE */}
              <button 
                onClick={onCartClick}
                className="p-3 bg-slate-100 text-slate-700 rounded-full hover:bg-slate-200 transition-all relative group"
                title="Your Cart"
              >
                <ShoppingCart className="w-6 h-6 group-hover:scale-110 transition-transform" />
                {cartCount > 0 && (
                  <span className="absolute -top-1 -right-1 w-6 h-6 bg-[#F7941D] text-white text-[11px] font-black rounded-full flex items-center justify-center border-2 border-white shadow-lg animate-in zoom-in-50 duration-300">
                    {cartCount > 9 ? '9+' : cartCount}
                  </span>
                )}
              </button>

              <button className="ml-2 px-8 py-3 nibbo-gradient text-white rounded-full font-black text-base shadow-lg hover:shadow-orange-200 transition-all active:scale-95">
                Sign In
              </button>
            </div>
          </div>
          ...
          <div className="md:hidden flex items-center gap-2">
            <button onClick={onCartClick} className="p-2 text-slate-600 relative active:scale-90 transition-transform">
              <ShoppingCart className="w-7 h-7" />
              {cartCount > 0 && (
                <span className="absolute top-1 right-1 w-5 h-5 bg-[#F7941D] text-white text-[10px] font-black rounded-full flex items-center justify-center border-2 border-white">
                  {cartCount}
                </span>
              )}
            </button>
            ...
            <button 
              ...
              className="p-2 text-slate-900 ml-1"
            >
      ...
      {/* Mobile Menu */}
      {mobileMenuOpen && (
        <div className="md:hidden absolute top-full left-0 right-0 bg-white border-t border-slate-100 p-8 shadow-2xl animate-in slide-in-from-top-4 duration-300 h-screen">
          <div className="flex flex-col space-y-6">
            <button onClick={onAboutClick} className="text-2xl font-black text-slate-900 py-3 border-b border-slate-50 text-left">About Nibbo</button>
            {navItems.map((item) => (
              <button 
                key={item.id}
                onClick={() => handleNavClick(item.id)}
                className="text-2xl font-black text-slate-900 py-3 border-b border-slate-50 text-left"
              >
                {item.label}
              </button>
            ))}
            <button className="w-full mt-8 nibbo-gradient text-white py-5 rounded-[2rem] font-black text-xl shadow-2xl">
              Sign In
            </button>
          </div>
        </div>
